...
impl TspSerializer {
    pub fn serialize_file(tsp: &Tsp, path: String) -> Result<(File, String), ParseTspError> {
        ...
        match {
            ...
            if tsp.weight_kind() != WeightKind::Undefined {
                ...
                writeln!(writer, "EDGE_WEIGHT_TYPE: {}", weight_kind)?;
            }
            ...
            if tsp.coord_kind() != CoordKind::Undefined {
                ...
                writeln!(writer, "NODE_COORD_TYPE: {}", coord_kind)?;
            }
            if tsp.disp_kind() != DisplayKind::Undefined {
                ...
                writeln!(writer, "DISPLAY_DATA_TYPE: {}", disp_kind)?;
            }
            ...
            Ok(_) => (),
            Err(_) => return Err(ParseTspError::Other("Troubles writing the file")),
        };
        ...
        if let Err(e) = is_ok {
            ...
            match File::open(&path) {
                ...
                Err(_) => Err(ParseTspError::Other("Couldn't open path")),
            }
        }
    }
}
