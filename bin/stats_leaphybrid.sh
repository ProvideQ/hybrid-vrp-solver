...
# Verwenden Sie den regulären Ausdruck, um die Zahlen zu finden
numbers=$(cat $file_path | grep -E -o 'sampler created took [0-9]+\.[0-9]+' | grep -E -o '[0-9]+\.[0-9]+')
...
# 'qpu_access_time': 187530

# Verwenden Sie den regulären Ausdruck, um die Zahlen zu finden
numbers=$(cat $file_path | grep -E -o "'qpu_access_time': [0-9]+" | grep -E -o '[0-9]+')
...
# Verwenden Sie den regulären Ausdruck, um die Zahlen zu finden
numbers=$(cat $file_path | grep -E -o "'run_time': [0-9]+" | grep -E -o '[0-9]+')
...
# Berechnen Sie den Durchschnitt
sampler_run_time=$(awk "BEGIN {print $sum / ($count * 1000000)}")
